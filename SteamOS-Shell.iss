...
Name: "{userdesktop}\Go to Gamemode"; Filename: "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ""{app}\go-gamemode-user.ps1"""; WorkingDir: "{app}"; IconFilename: "{app}\steamdeck-gaming-return.ico"
Name: "{group}\Go to Gamemode"; Filename: "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"; Parameters: "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "; WorkingDir: "{app}"; IconFilename: "{app}\steamdeck-gaming-return.ico"; Tasks: startmenu

; Start Menu uninstall entry
Name: "{group}\Uninstall SteamOS Shell"; Filename: "{uninstallexe}"; Tasks: startmenu

[UninstallRun]
; Ensure Explorer is restored on uninstall (run once)
Filename: "{sys}\reg.exe"; Parameters: "add ""HKCU\Software\Microsoft\Windows NT\CurrentVersion\Winlogon"" /v Shell /t REG_SZ /d explorer.exe /f"; Flags: runhidden; RunOnceId: restore_desktop_shell
...
begin
  SteamDirPage := CreateInputDirPage(
    ...
    'Select the folder that contains "steam.exe". We''ll use it to start Big Picture when Game Mode is enabled.',
    False, ''
  );
...
begin
  ...
  begin
    ...
    begin
      MsgBox(
        'steam.exe not found in:' + Chr(13) + Chr(10) + SteamDirPage.Values[0] +
...
begin
  ...
  begin
    ...
    S := '$sid = [System.Security.Principal.WindowsIdentity]::GetCurrent().User.Value' + #13#10 +
